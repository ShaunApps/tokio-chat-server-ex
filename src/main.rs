...
struct Peer {
    /// Name of the peer. first line received from the client
    name: BytesMut,

    /// The TCP socket wrapped with the `Lines` code. 
    lines: Lines,
    ...
    /// 
    /// This is used to receive messages from the peers. When a message
    /// is received off of this `Rx`, it will be written to the socket. 
    rx: Rx,

    /// Client socket address. 
    /// 
    /// The socket address is used as the key in the `peers` HashMap. The
    /// address is saved so that the `Peer` drop implementation can clean up
    /// its entry
    addr: SocketAddr,
}
...
impl Stream for Lines {
    ...
    fn poll(&mut self) -> Result<Async<Option<Self::Item>>, Self::Error> {
        // First, read any data that migh have been received from socket
        // 
        // track if socket is close here and will be used to inform the return 
        // value below
        ...
        if let Some(pos) = pos {
            // Remove the line from the read buffer and set it to
            // `line`
